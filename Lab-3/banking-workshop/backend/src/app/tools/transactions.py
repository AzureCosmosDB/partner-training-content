...
@tool
def bank_transfer(
    config: RunnableConfig, toAccount: str, fromAccount: str, amount: float
) -> str:
    """Wrapper function to handle the transfer of funds between two accounts."""
    # Debit the amount from the fromAccount
    debit_result = bank_transaction(
        config, fromAccount, amount, credit_account=0, debit_account=amount
    )
    if "Failed" in debit_result:
        return f"Failed to debit amount from {fromAccount}: {debit_result}"

    # Credit the amount to the toAccount
    credit_result = bank_transaction(
        config, toAccount, amount, credit_account=amount, debit_account=0
    )
...
def bank_transaction(
    ...
    if not account:
        print(
            f"Account {account_number} not found for tenant {tenantId} and user {userId}"
        )
    ...
    for attempt in range(max_attempts):
        try:
            ...
            print(
                f"Successfully transferred ${amount} to account number {account_number}"
            )
...
def get_transaction_history(
    ...
    try:
        ...
        logging.error(
            f"Error fetching transaction history for account {accountId}: {e}"
        )
        return []
